...
# tài khoản quản trị lấy từ biến môi trường LFOOD_ADMIN_LOGIN, LFOOD_ADMIN_PASSWORD (không ghi mật khẩu vào tệp)
$alogin = if ($env:LFOOD_ADMIN_LOGIN) { $env:LFOOD_ADMIN_LOGIN } else { 'hugoboss.v6@gmail.com' }
$apass = if ($env:LFOOD_ADMIN_PASSWORD) { $env:LFOOD_ADMIN_PASSWORD } else { 'admin' }
$r3 = Rpc $a '/web/session/authenticate' @{ db = 'lfood'; login = $alogin; password = $apass }
"Admin dang nhap uid: " + $r3.result.uid
foreach ($m in @(@('lfood.rnd.project', 'lfood_rnd.view_rnd_project_list'), @('lfood.rnd.formula', 'lfood_rnd.view_rnd_formula_form'), @('lfood.rnd.sample', 'lfood_rnd.view_rnd_sample_form'), @('res.users', 'lfood_admin.view_users_lfood_form'), @('lfood.backup', 'lfood_admin.view_backup_list'), @('lfood.backup.status', 'lfood_admin.view_backup_status_form'), @('res.partner', 'lfood_admin.view_partner_lfood_form'), @('lfood.role.matrix', 'lfood_admin.view_role_matrix_form'))) {
